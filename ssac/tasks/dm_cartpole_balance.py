...
# Load one task:
#This is not yet compatible with the environment class which is used in this implementation

def make(cfg):
    #env = suite.load(domain_name="cartpole", task_name="balance")
    #use github repo to convert dm_control environment into gymnasium environment
    env = dm_control2gym.make(domain_name="cartpole", task_name="balance")
    return env'''



#Yarden conversion code
...
from ssac.rl.wrappers import ImageObservation
...
def make(cfg: DictConfig) -> EnvironmentFactory:
    def make_env(cfg):
        ...
        if cfg.training.safe == True:
            env = ConstraintWrapper(env, cfg.training.slider_position_bound)
        from gymnasium.wrappers.flatten_observation import FlattenObservation
        env = FlattenObservation(env)

        #P: rest might be helpful when adding more environments
...
ENVIRONMENTS = {
    ("dm_cartpole", "balance"),
    ("dm_cartpole", "swingup"), #800-900 are good scores
